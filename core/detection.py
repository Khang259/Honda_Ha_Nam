# File: detection.py
from config import THRESHOLD_DETECT, THRESHOLD_COVERAGE
...
def has_object_in_roi(detections, roi, node_id=None, use_gpu=True):
    ...
    """
    try:
        threshold_coverage = THRESHOLD_COVERAGE #0.5
        threshold_detect = THRESHOLD_DETECT #0.4
        roi_box = roi
        ...
        if use_gpu and isinstance(detections, torch.Tensor):
            ...
            vehicle_mask = (detections[:, 5] == 0.0) & (detections[:, 4] > threshold_detect) #This line check the class_id of the object=0 and the confidence of the object
