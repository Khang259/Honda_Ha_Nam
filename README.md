...
**Mô tả ngắn gọn**: Service **AI runtime** xử lý nhiều luồng camera (RTSP), chạy suy luận (YOLO) theo cơ chế **batch + GPU**, quản lý **trạng thái theo node/ROI**, và cung cấp **FastAPI** để điều khiển/quan sát runtime và stream video (MJPEG).

Luồng khởi chạy chính: `api/main_ai.py` → chạy Uvicorn cho `api/ai_server.app`. Trong lifecycle, hệ thống kết nối MongoDB và khởi tạo runtime (`runtime_service.start()`).

**Sản phẩm này là gì**

- Một **backend service** chạy realtime: ingest video → inference → state machine theo ROI → API cho hệ sinh thái (UI/worker/monitoring).
...
- **Bật/tắt camera theo `cameraId`** khi cần bảo trì hoặc giảm tải hệ thống (API layer gọi `camera_control_service`).
- **Xem live view có overlay** để debug/tuning ROI và xác nhận detections (MJPEG streaming + vẽ ROI/detections).
- **Chạy theo worker**: một máy chỉ xử lý subset camera (lấy camera configs được gán từ MongoDB, heartbeat định kỳ).
- **Kiểm chứng hiệu năng**: dùng `unit_test/` để benchmark latency/batching/GPU decode trước khi đưa lên production.
...
- **Core runtime** (`core/`)
  - `core/worker_manager.py`: đăng ký/heartbeat + lấy camera configs theo worker
  ...
  - `core/state_manager.py`: state theo `start_*` / `end_*` và logic “ready”
  - `core/pair_manager.py`: xử lý nghiệp vụ start/end (gọi ICS/webhook theo cấu hình)
...
- **AI/Video**
  ...
  - OpenCV, PyAV
- **Frontend**
  - React: README template có nhắc tới, nhưng **repo hiện tại không chứa mã frontend** (nếu có frontend thì thường nằm ở repo/thư mục khác).
...
Repo này không có frontend. Nếu bạn có frontend riêng (React), hãy cấu hình gọi API theo base URL của backend (ví dụ: `http://<AI_SERVER_HOST>:<AI_SERVER_PORT>`).
...
Hiện tại repo **không có** `Dockerfile`/`docker-compose` trong workspace này. Nếu bạn muốn deploy theo Docker, cần bổ sung:

- Dockerfile cho backend (Python + CUDA/TensorRT nếu dùng GPU)
- compose cho MongoDB/Redis (nếu dùng)

## Troubleshooting

- **Không kết nối được MongoDB**: kiểm tra `MongoDB_URL`, network, user/password, và firewall.
- **Không có frame/stream trống**: kiểm tra camera RTSP URL trong config lấy từ MongoDB (worker assignment).
- **GPU inference không chạy**: kiểm tra driver CUDA, Torch CUDA, ONNXRuntime-GPU/TensorRT đúng phiên bản, và quyền truy cập GPU.
