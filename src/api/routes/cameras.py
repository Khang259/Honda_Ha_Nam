...
async def start_all_cameras() -> Dict[str, Any]:
    """Enable all cameras and resume inference (cluster-wide command)."""
    ...
    try:
        ...
        return {
            "code": 1000,
            "message": "Cluster start-all command accepted",
            "accepted": True,
            "token": result["token"],
            "desired_state": result["desired_state"],
            "updated_by": result["updated_by_worker"]
        }
    except Exception as e:
        logger.error(f"Error issuing start-all command: {e}")
        return {
            "code": 1001,
...
async def stop_all_cameras() -> Dict[str, Any]:
    """Disable all cameras and pause inference (cluster-wide command)."""
    from load_balancing.persistence.mongo_cluster_control_client import MongoClusterControlClient
    from api.settings import settings
    
    try:
        client = MongoClusterControlClient()
        result = await client.update_desired_state("STOP", settings.WORKER_ID)
        
        return {
            "code": 1000,
            "message": "Cluster stop-all command accepted",
            "accepted": True,
            "token": result["token"],
            "desired_state": result["desired_state"],
            "updated_by": result["updated_by_worker"]
        ...
        logger.error(f"Error issuing stop-all command: {e}")
        return {
            "code": 1001,
            "message": f"Error: {str(e)}",
            "success": False
...
async def get_cluster_status() -> Dict[str, Any]:
    """Get current cluster control status + worker acknowledgments."""
    from load_balancing.persistence.mongo_cluster_control_client import MongoClusterControlClient
    ...
    try:
        ...
        # Get acks for current token
        current_token = status.get("token", 0)
        acks = await client.get_acks(current_token)
        ...
        return {
            ...
            "acks": acks,
            "acks_count": len(acks),
            "expected_workers": 4  # Can be made dynamic if needed
        }
    except Exception as e:
