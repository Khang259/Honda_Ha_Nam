...
from inference_engine.snapshot_manager import SnapshotManager
from inference_engine.camera_manager import CameraManager
from inference_engine.inference_engine import InferenceEngine
from load_balancing.worker_manager import WorkerManager
...
class RuntimeService:
    def __init__(self):
        ...
        self._cluster_listener_task: Optional[asyncio.Task] = None
    ...
    async def start(self) -> RuntimeStatus:
        ...
        # Start cluster control listener (after all components initialized)
        from load_balancing.cluster_control_listener import ClusterControlListener
        self._cluster_listener = ClusterControlListener(worker_id=settings.WORKER_ID)
        self._cluster_listener_task = asyncio.create_task(
            self._cluster_listener.start()
        )
    ...
    async def stop(self) -> RuntimeStatus:
        ...
        # Stop cluster listener task
        if self._cluster_listener:
            try:
                await self._cluster_listener.stop()
            except Exception as e:
                print(f"Error stopping cluster listener: {e}")
        
        if self._cluster_listener_task and not self._cluster_listener_task.done():
            self._cluster_listener_task.cancel()
            try:
                await self._cluster_listener_task
        ...
        self._cluster_listener = None
        self._cluster_listener_task = None
